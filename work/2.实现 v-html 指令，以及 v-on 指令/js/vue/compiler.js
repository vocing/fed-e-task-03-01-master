...
class Compiler {
    ...
    onUpdater(node, attrName, key) {
        ...
        let exec = reg.exec(node.outerHTML)[1]
        const event = attrName.substr(3)
        // 判断是否入参
        const index = exec.search(/\(/)
        let argStr = ''
        let method = exec
        let args = []
        if (index + 1) {
            method = exec.slice(0, index)
            argStr = exec.slice(index + 1, exec.length - 1)
            args = argStr.split(",").map(item => item.trim())
        }
        node[`on${event}`] = (eventArgs) => {
            const current = args.map(item => {
                // $event参数转换
                if (item === '$event') {
                    return eventArgs.target.value
                } else {
                    return this.vm[item]
                }
            })
            // 合并参数
            const allEvent = current.concat(eventArgs)
            this.vm.$options.methods[method](...allEvent)
        }
    }
