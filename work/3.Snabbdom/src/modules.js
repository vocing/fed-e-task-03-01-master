var h = require('snabbdom/h.js').default;
...
const render = (newData, callback) => {
    originData = newData.reduce((arr, item) => {
        var last = arr[arr.length - 1];
        item.offset = last ? last.offset + last.elmHeight : 0;
...
function movieView(movie, index, data) {
    return h('div.row', {
        key: movie.rank,
        style: {
            opacity: '0',
    ...
    return h('div', [
        ...
        h('div', [
            h('a.btn.add', {
                on: {
                    click: () => {
                        const obj = JSON.parse(JSON.stringify(originalData[Math.floor(Math.random() * 10)]))
                        obj.rank = ++nextKey
                        render(add(data, [obj]))
                    }
                }
